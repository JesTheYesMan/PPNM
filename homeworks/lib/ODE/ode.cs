...
public static class ODE
{
    ...
    )
    {
        ...
        double x = a;
        vector y = ya.copy(), tol = new vector(y.size);
        ...
        do
        {
            ...
            for (int i = 0; i < y.size; i++)
                tol[i] = (acc + eps * Abs(yh[i])) * Sqrt(h / (b - a)); /* Evaluate the tolerances */

            bool ok = true;
            for (int i = 0; i < y.size; i++)
                if (!(err[i] < tol[i])) ok = false; /* check whether to accept step */
            ...
            for (int i = 1; i < y.size; i++)
                factor = Min(factor, tol[i] / Abs(err[i])); /* figure out new step size */
