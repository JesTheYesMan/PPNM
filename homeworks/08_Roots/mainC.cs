...
public class main{
	public static int Main(){
		...
		Func<vector,vector> f1 = delegate(vector v){vector w = new vector(1); 
		...
		matrix xs = new matrix("-0.5 0 0.5");
                ...
                for(int i=0;i<3;i++){
                        vector x = xs[i];
			root.newton_quad_int root = new root.newton_quad_int(f1,x);
			vector xmin = root.x;
			vector fmin = root.f;
		...
		Func<vector,vector> f2 = delegate(vector v){vector w = new vector(2); 
                ...
                for(int i=0;i<3;i++){
                        vector x = xs2[i];
                        root.newton_quad_int root = new root.newton_quad_int(f2,x);
                        vector xmin = root.x;
                        vector fmin = root.f;
                        int steps = root.steps;
                        int f_eval = root.f_eval;
		...
		Func<vector,vector> f3 = delegate(vector v){vector w = new vector(2);
		...
		matrix xs3 = new matrix("0.5 1.2 2");
                ...
                for(int i=0;i<3;i++){
                        vector x = xs3[i];
                        root.newton_quad_int root = new root.newton_quad_int(f3,x);
                        vector xmin = root.x;
                        vector fmin = root.f;
                        int steps = root.steps;
                        int f_eval = root.f_eval;
		...
		for(int i=0;i<3;i++){
			vector x0 = x0s_R[i];
                        root.newton_quad_int root = new root.newton_quad_int(dRf,x0);
                        vector xmin = root.x;
                        vector dfmin = root.f;
                        int steps = root.steps;
                        int f_eval = root.f_eval;
		...
		//Himmelblau function part
                ...
                Func<vector,vector> dHf = delegate(vector v){vector w = new vector(2);
		...
		WriteLine("f(vmin) = 0, df(vmin) = 0");
                ...
                for(int i=0;i<4;i++){
                        vector x0 = x0s_H[i];
                        root.newton_quad_int root = new root.newton_quad_int(dHf,x0);
                        vector xmin = root.x;
                        vector dfmin = root.f;
                        int steps = root.steps;
                        int f_eval = root.f_eval;
