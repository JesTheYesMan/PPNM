...
public static (vector eigenvalues, matrix V) generalised_eigenvalues_solver(matrix A, matrix B) {
	...
	//Finding A_tilde
    matrix A_tilde = S_inv_sqrt * Q.transpose() * A * Q * S_inv_sqrt;
    	A_tilde.print("\nA_tilde = ");
...
static (vector Eigenvalues, matrix Eigenvectors) SolveGeneralizedEigenproblem(matrix H, matrix N)
    {
        // Cholesky decomposition of N
        var cholesky = N.Cholesky();

        // Transform the problem to a standard eigenvalue problem
        matrix<double> Linv = cholesky.Factor.Inverse();
        matrix<double> Linv_H = Linv * H * Linv.Transpose();

        // Solve the standard eigenvalue problem
        var evd = Linv_H.Evd(Symmetricity.Symmetric);

        // Transform eigenvectors back to the original basis
        matrix<double> eigenvectors = Linv.Transpose() * evd.EigenVectors;

        return (evd.EigenValues.Real(), eigenvectors);
    }
    ...
    static (double Energy, vector Coefficients) VariationalHydrogenGroundState(double[] alphas)
    {
        SetupMatrices(alphas, out matrix<double> H, out matrix<double> N);
        var (eigenvalues, eigenvectors) = SolveGeneralizedEigenproblem(H, N);

        // Ground state energy
        double groundStateEnergy = eigenvalues[0];
        vector<double> groundStateVector = eigenvectors.Column(0);

        return (groundStateEnergy, groundStateVector);
    }
